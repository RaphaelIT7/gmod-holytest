...
static IGMod_Audio* gGModAudio = nullptr;
...
void CBassModule::Init(CreateInterfaceFn* appfn, CreateInterfaceFn* gamefn)
{
	...
	if (gGModAudio)
		gGModAudio->Init(*appfn); // The engine didn't initialize it...
}
...
extern CGlobalVars* gpGlobals; // Always set - See plugin.cpp CServerPlugin::Load
void CBassModule::Think(bool bSimulating)
{
	if (gGModAudio && gpGlobals) // Just to be sure, you never know...
